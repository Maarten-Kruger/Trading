...
//+------------------------------------------------------------------+
//| Triggered for order/position/deal events                         |
//+------------------------------------------------------------------+
void OnTradeTransaction(const MqlTradeTransaction &trans,
                        const MqlTradeRequest &request,
                        const MqlTradeResult  &result)
{
   // We only care about new deals being registered.
   if(trans.type != TRADE_TRANSACTION_DEAL_ADD)
      return;

   // Pull the deal and ensure it's a close (EXIT) for OUR magic.
...
//+------------------------------------------------------------------+
//| Log equity when a trade closes                                   |
//+------------------------------------------------------------------+
void LogEquityOnClose(const datetime when)
{
   const double eq = AccountInfoDouble(ACCOUNT_EQUITY);

   int h = FileOpen("EquityOnClose.csv",
                    FILE_READ | FILE_WRITE | FILE_CSV | FILE_ANSI, ";");
   if(h == INVALID_HANDLE) return;

   if(FileSize(h) == 0)
      FileWrite(h, "DateTime", "Equity");

   FileSeek(h, 0, SEEK_END);
   FileWrite(h, TimeToString(when, TIME_DATE | TIME_SECONDS), eq);
   FileClose(h);
}
