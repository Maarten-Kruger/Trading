...
// --- New Risk Strategy Inputs ---
input double InpMaxEquityDD   = 4.0;    // 1. Global Equity Stop (%)
input double InpBEActivation  = 30.0;   // 2. Break-Even Activation (% to TP)
input double InpPartialScale  = 50.0;   // 2. Partial Close Size (%)
input double InpBaseATR       = 5.0;    // 4. Base ATR for Lot Scaling (Adjust based on your timeframe)

//--- GLOBALS
double PipPoint;
double GlobalPeakEquity = 0.0;
CTrade trade;
...
int OnInit() {
   ...
   // Modify this existing block in OnInit():
   if(InpRiskMode == RISK_WEEKDAYS_ATR_TRAIL || InpRiskMode == RISK_WEEKDAYS_HOLD_NEG) {
...
void OnTick() {
   ...
   // 1. GLOBAL EQUITY CIRCUIT BREAKER
   if(InpRiskMode == RISK_WEEKDAYS_HOLD_NEG) {
      double currentEquity = AccountInfoDouble(ACCOUNT_EQUITY);
      if(currentEquity > GlobalPeakEquity || PositionsTotal() == 0) {
         GlobalPeakEquity = currentEquity; // Reset peak
      }
      
      double currentDD = ((GlobalPeakEquity - currentEquity) / GlobalPeakEquity) * 100.0;
      if(currentDD >= InpMaxEquityDD && PositionsTotal() > 0) {
         Print("Global Equity Stop Hit! Closing all trades.");
         for(int i = PositionsTotal() - 1; i >= 0; i--) {
            ulong ticket = PositionGetTicket(i);
         ...
         GlobalPeakEquity = AccountInfoDouble(ACCOUNT_EQUITY); 
         return; // Stop further processing this tick
      }
   }

   // 2. BREAK-EVEN & PARTIAL SCALING
   if(InpRiskMode == RISK_WEEKDAYS_HOLD_NEG) {
      for(int i = PositionsTotal() - 1; i >= 0; i--) {
         ulong ticket = PositionGetTicket(i);
         if(PositionSelectByTicket(ticket) && PositionGetInteger(POSITION_MAGIC) == InpMagicNum) {
            ...
            if(PositionGetInteger(POSITION_TYPE) == POSITION_TYPE_BUY) {
               double totalDistance = tpPrice - openPrice;
               double currentDistance = currentPrice - openPrice;
               
               // If we reached the BE activation percentage
               if(totalDistance > 0 && (currentDistance / totalDistance) * 100.0 >= InpBEActivation) {
                  // Use SL position to check if we've already done this step
                  if(slPrice < openPrice) { 
                     // Move SL to Entry
                     trade.PositionModify(ticket, openPrice, tpPrice);
                     
                     // Scale out partial position
                     double closeVol = volume * (InpPartialScale / 100.0);
                     double lotStep = SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_STEP);
                     closeVol = MathFloor(closeVol / lotStep) * lotStep; // Normalize for broker
                     
                     if(closeVol >= SymbolInfoDouble(_Symbol, SYMBOL_VOLUME_MIN)) {
                        trade.PositionClose(ticket, closeVol); 
                     }
                  }
               }
            }
      ...
      for(int i = PositionsTotal() - 1; i >= 0; i--) {
         ...
         if(PositionSelectByTicket(ticket)) {
            ...
            if(magic == InpMagicNum || magic == (InpMagicNum + 1)) {
               ...
               else if(InpRiskMode == RISK_WEEKDAYS_HOLD_NEG && profit > 0) {
   ...
   if(currentBar != lastBar) {
      ...
      if(mainTradesCount < InpMaxOpenTrades) {
         ...
         if(triggerMet) {
            ...
            double finalLotSize = InpLots;
            
            if(InpRiskMode == RISK_WEEKDAYS_HOLD_NEG) {
               double atrVal[];
               ArraySetAsSeries(atrVal, true);
               if(CopyBuffer(hATR, 0, 0, 1, atrVal) > 0) {
                  // Scale inversely: (Base ATR / Current ATR) * Base Lots
                  finalLotSize = (InpBaseATR / atrVal[0]) * InpLots;
            ...
            // Replace the original trade.Buy line with this:
            if(trade.Buy(finalLotSize, _Symbol, ask, sl, tp, EnumToString(InpTriggerMode))) {
               TotalOpened++;
            }
