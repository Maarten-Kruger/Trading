...
def run_strategy(rows: List[Dict[str, float]], range_period: int, threshold: float, risk: float, reward: float, spread: float) -> List[float]:
    trades: List[float] = []
    i = range_period
    while i < len(rows):
        recent_high = max(r['High'] for r in rows[i-range_period:i])
        recent_low = min(r['Low'] for r in rows[i-range_period:i])
        if recent_high - recent_low <= threshold:
            price = rows[i]['Close']
...
def sensitivity_analysis(rows: List[Dict[str, float]], range_period: int, threshold: float, risk: float, rr_values: List[float], spread: float) -> List[Dict[str, float]]:
    results = []
    for rr in rr_values:
        trades = run_strategy(rows, range_period, threshold, risk, risk * rr, spread)
...
def main():
    ...
    parser.add_argument('--threshold', type=float, default=0.0003, help='Maximum range width')
    ...
    trades = run_strategy(rows, args.period, args.threshold, args.risk, args.risk * args.rr, args.spread)
    metrics = analyze_trades(trades)
    ...
    sa_results = sensitivity_analysis(rows, args.period, args.threshold, args.risk, rr_values, args.spread)
